    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-6">
        <div v-if="room" class="flex gap-6 text-text-primary">
          <p><span class="font-bold">{{ $t('nav.players') }} :</span> {{ room.users.length }}</p>
          <p><span class="font-bold">{{ $t('nav.timeLeft') }} :</span> {{ formatTime(room.timer) }}</p>
        </div>
      </div>
      ...
      <nav class="flex items-center gap-4">
        ...
        <a
          ...
          class="p-2 rounded-lg transition-colors bg-surface-alt hover:bg-surface text-text-primary"
          title="GitHub"
        >
  ...
  <main class="flex-1 h-full flex flex-col space-y-8 relative">
    <div v-if="!room" class="max-w-7xl mx-auto py-8">
      <div class="text-center text-text-primary">
        <p class="text-2xl">{{ isReconnecting ? $t('loading.reconnecting') : $t('loading.loadingGame') }}</p>
      </div>
    </div>
